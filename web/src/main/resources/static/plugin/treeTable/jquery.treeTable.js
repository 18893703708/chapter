...
(function ($) {
    ...
    $.fn.treeTable = function (opts) {
        ...
        //扩展方法 解决VUE与JQUERY事件冲突问题
        $treeTable.subClick = function(item) {
        ...
        $treeTable.setMouseover = function(item) {

            setActiveNode(item);


        }
        $treeTable.setMouseout = function(item) {

            setActiveNode(item);

        }
        function setActiveNode(item) {
            var $target = $(item);
            if ($target.attr('controller')) {
                $target = $target.parents('tr[haschild]').find('[arrow]');
            }
